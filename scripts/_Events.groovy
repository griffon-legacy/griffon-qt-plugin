...
eventCreateConfigEnd = {
    buildConfig.griffon.application.mainClass = 'griffon.qt.QtApplication'
    if (!buildConfig.griffon.app.javaOpts) buildConfig.griffon.app.javaOpts = []
    if (!buildConfig.griffon.app.javaOpts.contains('-XstartOnFirstThread')) {
    ...
    if (!isPluginProject) {
        ...
        if (!runtimeConfig.platform.handler.macosx) {
            runtimeConfigFile.append("\nplatform.handler.macosx = 'org.codehaus.griffon.runtime.qt.DefaultMacOSXPlatformHandler'")
        }
        if (!runtimeConfig.platform.handler.macosx64) {
            runtimeConfigFile.append("\nplatform.handler.macosx64 = 'org.codehaus.griffon.runtime.qt.DefaultMacOSXPlatformHandler'")
        }
    }
}
